...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\Zsuzsa\\HCCCL\\Prediction-and-Memory\\Hippopolis\\TwoCities_4AFCT_lastrun.py',
...
clock0 = visual.ImageStim(
    win=win,
    name='clock0', 
    image='stimuli\\\\stopwatch0.png', mask=None,
    ori=0.0, pos=(700, 400), size=(150, 150),
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
...
for thisCityBlock in CityBlock:
    ...
    for thisLearningTrial in LearningTrials:
        ...
        _key_resp_allKeys = []
        # keep track of which components have finished
        test_trialComponents = [plate_updating, city_text_updating, street_text_updating, test_image1, test_image2, test_image3, test_image4, button_d, button_f, button_j, button_k, key_resp, clock0]
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if clock0.status == STARTED:
                ...
                if tThisFlipGlobal > clock0.tStartRefresh + 1.5-frameTolerance:
                    ...
                    win.timeOnFlip(clock0, 'tStopRefresh')  # time at next scr refresh
        ...
        if response == 'dist2':
            dist2 += 1
        LearningTrials.addData('clock0.started', clock0.tStartRefresh)
        LearningTrials.addData('clock0.stopped', clock0.tStopRefresh)
